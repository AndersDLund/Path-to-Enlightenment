...
var state, robo, love, nicks, chimes, exit, player, dungeon,
    Ghandi, healthBar, message, gameScene, gameOverScene, enemies, id;
...
function setup() {
  ...
  for (var i = 0; i < numberOfnicks; i++) {
    ...
    //Space each nick horizontally according to the `spacing` value.
    //`xOffset` determines the point from the left of the screen
    //at which the first nick should be added
    ...
    nick.x = x;
    nick.y = y;
    //Set the nick's vertical velocity. `direction` will be either `1` or
    //`-1`. `1` means the enemy will move down and `-1` means the nick will
    //move up. Multiplying `direction` by `speed` determines the nick's
    //vertical direction
  ...
  left.press = function() {
    //Change the robo's velocity when the key is pressed
    robo.vx = -5;
    robo.vy = 0;
  };
  ...
  left.release = function() {
    ...
    //and the robo isn't moving vertically:
    //Stop the robo
...
function play() {
  ...
  //contain(robo, stage);
  //Set `roboHit` to `false` before checking for a collision
  var roboHit = false;
  //Loop through all the sprites in the `enemies` array
  nicks.forEach(function(nick) {
    //Move the nick
    nick.y += nick.vy;
    //Check the nick's screen boundaries
  ...
  //Check for a collision between the robo and the love
